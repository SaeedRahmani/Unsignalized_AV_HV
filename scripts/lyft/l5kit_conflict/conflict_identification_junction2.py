...
for scene_tuple_index, scene_tuple in enumerate(scene_tuples):
	...
	# if scene_tuple_index % 10 == 0:
	print(f"---- {scene_tuple_index} ----")
	
	# if scene_tuple_index in [364,]: # 255, 486, 1298, 1831, 2024, 2087, 2443, 4364,
	# 	continue
	...
	# print(np.array(ego_trajectory_intersection_lineString.coords.xy).shape)
	if np.array(ego_trajectory_intersection_lineString.coords.xy).shape[1] == 0:
		continue
		...
		for agent1_id in HV_turnleft_from_top:
			for agent2_id in HV_straight2left:
				...
				if is_conflict:
					...
					HVHV_conflict_v2["cross"]["goLeft&turnLeftFromTop"].append({scene_tuple: conflict})
		...
		for agent1_id in HV_straight2left:
			for agent2_id in HV_turnright_from_top:
				...
				if is_conflict:
					...
					HVHV_conflict_v2["merge"]["goLeft&turnRightFromTop"].append({scene_tuple: conflict})
		...
		for agent1_id in HV_straight2right:
			for agent2_id in HV_turnleft_from_top:
				...
				if is_conflict:
					...
					HVHV_conflict_v2["merge"]["goRight&turnLeftFromTop"].append({scene_tuple: conflict})
...
# %% save as pickle
save_pickle(obj=AVHV_conflict_v2, name=f"AVHV_conflict_{dataset_name}_{int(delta_time//10)}_{margin}_v2")
save_pickle(obj=HVHV_conflict_v2, name=f"HVHV_conflict_{dataset_name}_{int(delta_time//10)}_{margin}_v2")
