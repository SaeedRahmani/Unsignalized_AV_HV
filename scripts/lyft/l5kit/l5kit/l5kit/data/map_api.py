...
class MapAPI:
    ...
    @staticmethod
    @no_type_check
    def is_node(element: MapElement) -> bool:
        """
        Check whether an element is a valid node

        Args:
            element (MapElement): a proto element

        Returns:
            bool: True if the element is a valid node
        """
        return bool(element.element.HasField("node"))

    @lru_cache(maxsize=CACHE_SIZE)
    def get_junction_coords(self, element_id: str) -> dict:
        """
        Get XYZ coordinates in world ref system for a lane given its id
        lru_cached for O(1) access
