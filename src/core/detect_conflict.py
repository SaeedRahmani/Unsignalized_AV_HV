...
def detect_conflict_between_two_trajectories(
    ...
    tfrecord_index, scenario_index, intersection_circle: Polygon,
    center, radius,
    pet_threshold: float = 10, buffer: float = 1
):
    inbound_lane_linestring_list = build_lane_segments(inbound_lanes)
    outbound_lane_linestring_list = build_lane_segments(outbound_lanes)
...
#     @return: time_a_at_conflict: float, the timestamp of trajectory a reaching the objects point.
#     @return: time_b_at_conflict: float, the timestamp of trajectory b reaching the objects point.
#     @return: signed_pet: float, the difference between two trajectories' timestamp at objects point,
