...
from l5kit.data import get_frames_slice_from_scenes, ChunkedDataset
from l5kit.dataset.utils import convert_str_to_fixed_length_tensor

from .sampler import generate_agent_sample
...
class IntersectionDataset(Dataset):
	...
	def __init__(
		...
		# build a partial thus we don't have to access cfg each time
	...
	def get_scene_dataset(self, scene_index: int) -> "IntersectionDataset":
		"""
		...
		return IntersectionDataset(self.cfg, dataset, self.rasterizer, self.perturbation)
